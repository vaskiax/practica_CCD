# scripts/3_ganancia_ruido_manual_iraf.py
import os
import math
...
# Usaremos la carpeta de resultados para guardar estos archivos intermedios
...
# Dividimos los bias en dos grupos
bias_group1 = bias_files[:n_bias//2]
bias_group2 = bias_files[n_bias//2:]

# Combinamos cada grupo para crear B1 y B2
...
# --- 3. OBTENER TODAS LAS ESTADÍSTICAS ---
print("\n--- Obteniendo estadísticas de las imágenes combinadas ---")
# Obtenemos el tamaño de una de las imágenes para definir la región
...
# Obtenemos las medias de B1 y F1 (las de B2 y F2 son similares)
media_B1, _ = get_stats(B1_path, region)
media_F1, _ = get_stats(F1_path, region)

# Obtenemos las desviaciones estándar de las imágenes de diferencia
...
# La fórmula usa F1+F2 y B1+B2, pero como son promedios, podemos aproximar 2*media_F1 y 2*media_B1
# O, para ser más precisos, podemos calcular la media de F2 y B2 también
...
# Fórmula del Ruido de Lectura (RN)
